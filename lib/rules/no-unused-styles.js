...
module.exports = Components.detect((context, components) => {
  ...
  */
  const segmentPickerContexts = [];
  return {
    JSXOpeningElement: function (node) {
      if (node.name.name === 'SegmentPicker') {
        const possibleSegmentPickerStylesheetNames = node.attributes.map(
          attr => (((attr || {}).value || {}).expression || {}).name
        );
        if (possibleSegmentPickerStylesheetNames.some(astHelpers.isStyleSheetName)) {
          const segmentPickerInnerExpression = node.parent.children.find(n => n.type === 'JSXExpressionContainer')
            .expression;
          const segmentPickerExpressionParameterName = segmentPickerInnerExpression.params[0].name;
          segmentPickerContexts.push({
            parameterName: segmentPickerExpressionParameterName,
            stylesheetNames: possibleSegmentPickerStylesheetNames,
    ...
    JSXClosingElement: function (node) {
      if (node.name.name === 'SegmentPicker' && segmentPickerContexts.length > 0) {
        segmentPickerContexts.pop();
      }
    },
    ...
    MemberExpression: function (node) {
      ...
      if (isPotentialStyleRef) {
        ...
        if (segmentPickerContexts.length > 0) {
          // This block resolves the cases in which the ref is found inside a SegmentPicker block
          const segmentPickerStyleRefs = [];
          segmentPickerContexts.forEach((segmentPickerContext) => {
            if (
                segmentPickerContext.isStyleSegmentPicker &&
                node.object.name === segmentPickerContext.parameterName
              ) {
              segmentPickerContext.stylesheetNames.forEach((stylesheetName) => {
                segmentPickerStyleRefs.push([stylesheetName, node.property.name].join('.'));
              });
            }
          });
          if (segmentPickerStyleRefs.length === 0) {
            ...
            segmentPickerStyleRefs.push(baseRef);
          }
          segmentPickerStyleRefs.forEach((styleRef) => styleReferences.add(styleRef));
        } else {
